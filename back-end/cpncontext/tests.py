...
class SimpleTest(TestCase):
    ...
    def test_update_cpncontext_byid(self):
        response = self.client.put('http://127.0.0.1:8000/cpncontext/api/', data={'cid': '4', 'name': 'EtherGame Context', 'content': 'This is Content 3', 'context_type': 'type4', 'description': 'This is description'})
        self.assertEqual(response.status_code, status.HTTP_202_ACCEPTED)

    #Test delete cpncontext
    def test_delete_cpncontext(self):
        response = self.client.delete('http://127.0.0.1:8000/cpncontext/api/', data={'cid': '3'})
